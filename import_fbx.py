...
class FbxImportHelperNode:
    ...
    def build_skeleton(self, arm, parent_matrix, settings, parent_bone_size=1, target_parent_matrix=None):
        ...
        source_local_matrix = self.get_bind_matrix().normalized()
        bone_matrix = parent_matrix @ source_local_matrix

        bone.matrix = bone_matrix

        if settings.bone_orientation_mode == 'BETTER_FBX':
            if target_parent_matrix is None:
                target_parent_matrix = Matrix()
            child_heads = [
            ...
            if child_heads:
                ...
                target = bone.head + (bone.head - bone.parent.head) * 0.1
            else:
                target = bone.tail
            if (target - bone.head).length > 1e-3:
                bone.tail = target
            else:
                bone.tail = bone.head + bone_matrix.to_3x3() @ Vector((0.0, 1.0, 0.0)) * 0.01

            target_local_matrix = target_parent_matrix.inverted_safe() @ bone.matrix
            orientation_correction = source_local_matrix.inverted_safe() @ target_local_matrix
            self.post_matrix = (self.post_matrix if self.post_matrix else Matrix()) @ orientation_correction
        ...
        for child in self.children:
            if child.is_leaf and force_connect_children:
                ...
                child_bone = child.build_skeleton(
                    arm, bone_matrix, settings, bone_size,
                    bone.matrix if settings.bone_orientation_mode == 'BETTER_FBX' else None,
                )
    ...
    def set_pose_matrix_and_custom_props(self, arm, settings):
        ...
        pose_bone.bone["unity_fbx_path"] = self.source_bone_path()
    ...
    def source_bone_path(self):
        names = []
        node = self
        while node and node.is_bone:
            names.append(node.fbx_name)
            node = node.parent
...
def _bake_matrix_into_object_data(obj, matrix):
    """Bake `matrix` into `obj`'s data. Returns False when the data cannot be baked safely."""
    data = obj.data
    if data is None:
        # Empties carry no data, clearing the object transform alone is enough.
        return True
    if not hasattr(data, "transform") or data.users > 1:
        return False
    if isinstance(data, (bpy.types.Mesh, bpy.types.Curve, bpy.types.Lattice)):
        data.transform(matrix, shape_keys=True)
    else:
...
def bake_imported_root_axis_conversion(root_helper, view_layer, report, bake_all=False):
    ...
    for node in collect_root_nodes(root_helper):
        ...
        for child in obj.children:
            ...
            # local transform into single-user geometry as well, so meshes such as
            # Body do not retain an X=-90 degree object rotation.
            child_matrix = child.matrix_basis.copy()
            if (child.type in {'MESH', 'CURVE', 'LATTICE'} and child_matrix != Matrix()
                    and _bake_matrix_into_object_data(child, child_matrix)):
                child.matrix_basis = Matrix()

    view_layer.update()

    if skipped:
        report({'WARNING'}, rpt_("Could not bake the axis conversion of: %s") % ", ".join(skipped))
